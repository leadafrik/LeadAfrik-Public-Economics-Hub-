  ...
  return (
    <article className="border border-gray-200/60 rounded-lg p-8 hover:shadow-lg hover:border-amber-600 transition-all duration-200 bg-white">
      ...
      <h3 className="text-2xl font-serif font-bold text-gray-900 mb-3 leading-tight">
        <Link href={`/documents/${slug}`} className="hover:text-amber-600 transition-colors">
          {title}
        </Link>
      </h3>
      ...
      <Link
        href={`/documents/${slug}`}
        className="text-sm font-semibold text-amber-600 hover:text-amber-700 transition-colors inline-flex items-center gap-2"
      >
        View Document
        <span>→</span>
      </Link>
